...
async function fetchSummary() {
  ...
  const apiCall = `https://collab-connect-service-57pbrco4da-uc.a.run.app/getSummary/${docId}`;
...
async function fetchLockedPosts() {
  ...
  const apiCall = `https://collab-connect-service-57pbrco4da-uc.a.run.app/getAll/${docId}`;
  const response = await fetch(apiCall);
  const posts = await response.json();
  return posts;
}
...
async function deletePost(id) {
  console.log(id);
  const apiCall = `https://collab-connect-service-57pbrco4da-uc.a.run.app/deletePost/${id}`;
...
async function getAllEditors() {
  ...
  const apiCall = `https://collab-connect-service-57pbrco4da-uc.a.run.app/getEditors/${docId}`;
  const response = await fetch(apiCall);
  const posts = await response.json();
  return posts;
}
